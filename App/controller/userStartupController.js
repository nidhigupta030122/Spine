...
module.exports.signup = async (req, res) => {
  ...
  const { role, startupName, email, password, password_confirmation, mobile_token,session } = req.body;
  console.log(".....................", req.body);
  if (email && password) {
    const user = await UserModel.findOne({ email: email })
    const investor = await investorModel.findOne({ email: email })
    if(investor){
      res.status(201).send({ "success": false, "status": "201", "message": "You are register as a Investor User" })
    }
...
module.exports.Login = async (req, res) => {
  try {
    ...
    if (password && email) {
      ...
      if (!data) {
        return res.status(401).send({ "success": false, "status": "401", "message": "Email is not Valid for startup user" })
      }
      if (data.role == role) {
        if (data != null) {
          ...
          if(data.otp_verified===false){
            ...
            // Generate JWT Token
            const token = jwt.sign({ userID: data._id }, process.env.JWT_SECRET_KEY, { expiresIn: '5d' })
            return res.status(200).send({ "success": true, "status": "200", "message": "Login Successfully", data, "token": token })
          } else {
            return res.status(401).send({ "success": false, "status": "401", "message": "Email or Password is not Valid" })
          }
        } else {
          return res.status(200).send({ "success": false, "status": "401", "message": "You are not a Registered User" })
        }
      } {
        return res.status(200).send({ "success": false, "status": "401", "message": "You are not startup user" })
      }
    } else {
      return res.status(401).send({ "success": false, "status": "401", "message": "Email or Password are Required" })
    }
  } catch (error) {
    console.log(error)
    return res.status(401).send({ "success": false, "status": "401", "message": "Unable to Login" })
  }
}
...
module.exports.Socialsignup = async (req, res) => {
  ...
  if (checkemail) {
    return res.status(200).send({ "success": false, "Status": "401", "message": "You are register as a Investor User" })
  }
  try {
    if (!users) {
      if (user) {
...
module.exports.socialLogin = async (req, res) => {
  try {
    ...
    if (data.length == 0) {
      ...
      const datas = await UserModel.findOne({ $and: [{ email: email }, { social_id: social_id }] });
      ...
      if (datas.role == role) {
        ...
        res.status(401).send({ "success": false, "status": "401", "message": "You are not Startup user" })
      }
...
module.exports.setPassword = async (req, res) => {
  ...
  try {
    ...
    if (password === password_confirmation) {
      const saved_user = await UserModel.findOneAndUpdate({ email: email }, { $set: { password: newHashPassword } })
      if (saved_user) {
...
module.exports.updateProfile = async (req, res) => {
  try {
    var { description, email, startupName, startupStage, pitchDeckLink, founderName, chooseIndustry, location, teamSize, BusinessModel, fundRaise, pitchDeck } = req.body;
    const data = await UserModel.findOneAndUpdate({ _id: req.user._id },
      {
        $set: {
          ...
          profile_pic: req?.files?.profile_pic == undefined ? "" : req?.files?.profile_pic[0]?.location,
          ...
          fundRaise: fundRaise,
          pitchDeck: req?.files?.pitchDeck == undefined ? "" : req?.files?.pitchDeck[0]?.location,
...
module.exports.setIndustry = async (req, res) => {
  try {
    ...
    if (data) {
      res.status(200).send({ "success": true, "status": "200", "message": "update setIndustry  succesfully", data })
    } else {
      res.status(401).send({ "success": false, "status": "401", "message": "Something Went Wrongs" })
    }
  } catch (err) {
...
module.exports.setfundingTeam = async (req, res) => {
  try {
    ...
    if (data) {
      res.status(200).send({ "success": true, "status": "200", "message": "update setFundingTeam succesfully", data })
    } else {
      res.status(401).send({ "success": false, "status": "401", "message": "Something Went Wrongs" })
    }
  } catch (err) {
...
module.exports.setfundRaising = async (req, res) => {
  try {
    ...
    if (data) {
      res.status(200).send({ "success": true, "status": "200", "message": "update setFundraise succesfully", data })
    } else {
      res.status(401).send({ "success": false, "status": "401", "message": "Something Went Wrongs" })
    }
  } catch (err) {
...
module.exports.startupMyProfile = async (req, res, next) => {
  try {
    const { startupName, founderName, email, description,chooseIndustry,startupStage,teamSize,BusinessModel } = req.body;
    ...
    const updareprofile = await UserModel.findByIdAndUpdate({ _id: req.user._id }, {
      $set: {
...
module.exports.changeStartupPassword = async (req, res, next) => {
  try {
    ...
    if (comparePassword == true) {
      ...
      const changePassword = await UserModel.findByIdAndUpdate({ _id: req.user._id }, {
        $set: {
          password: hashPassword
        }
      }, { new: true })
...
module.exports.fetchNotification = async (req, res, next) => {
  try {
    const fetchNotification = await notification.find().populate("user_id")
...
//   to:user_id
// })
const startUpRequestAccept=await notification.findByIdAndUpdate({_id:_id}, { $set: { status: "accept",title:"Stated following you" } })
    var message = {
      to: User.mobile_token,
      notification: {
        title: 'notification',
...
module.exports.rejectedRequest = async (req, res, next) => {
  try {
    ...
    const rejectRequest = await notification.findByIdAndDelete({ _id: _id });
    return res.status(200).json({
      status: true,
